...
const DiscoverCard = ({ user }: { user: any }) => {
  const { data: statusData, isLoading: connectionLoading } =
    useGetConnectionStatus(user.id)
...
export const PeoplePage = memo(() => {
  ...
  const users = (usersPage?.data ?? []).filter(
    ...
    <div className="mx-auto max-w-5xl space-y-6 px-4 py-8">
      <Card className="border-border/70 bg-card/80">
        <CardHeader className="pb-4">
          <CardTitle className="text-3xl tracking-tight">People</CardTitle>
        </CardHeader>
      </Card>
